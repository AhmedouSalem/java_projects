...
public class JavaMailUtil {
	...
	private static Message prepareMessage(Session session, String myAccountEmail, String recipientEmail) {
		...
		try {
			...
			message.setSubject("Bienveu sur market place");
			message.setText("Bonjour! " + recipientEmail + ", Merci de faire le shopping chez nous!");
			return message;
	...
	private static Message prepareMessage(Session session, String myAccountEmail, String recipientEmail, String subject,
		...
		try {
			...
			if (session == null || myAccountEmail == null || recipientEmail == null || subject == null
					|| htmlTextMessage == null) {
				throw new IllegalArgumentException("Un ou plusieurs paramètres sont nuls");
			}
			...
			Logger.getLogger(JavaMailUtil.class.getName()).log(Level.SEVERE, "Erreur lors de la préparation du message",
					exception);
			throw new RuntimeException("Erreur lors de la préparation du message", exception);
		}
	}
